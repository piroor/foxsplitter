...
shouldAutoHideToolbox=Hide toolbar automatically in normal (not main) split windows
...
hiddenUIInMemberWindow=Show following UI items in normal (not main) split windows
...
shouldScrollToSplitPosition=String scrolled position of windows which are split from one tab
shouldKeepSizeRatioOnResize=Resize split windows to keep their size ratio, when one of them is resized

platform.button=Configurations of workarounds for platform specific issues
platform.title=Workarounds for platform specific issues

platformOffset.caption=Offset of positioning and resizing
...
methodToRaiseWindow.caption=How to raise grouped windows to front
methodToRaiseWindow.zLevel=By nsIXULWindow::zLevel (recommended for Windows)
methodToRaiseWindow.wmctrl=By wmctrl (recommended for Linux)
methodToRaiseWindow.wmctrl.path=Path to wmctrl:
methodToRaiseWindow.focus=By window focus
methodToRaiseWindow.none=Do nothing (grouped windows can hide behind another window)
...
ui.stretch.long=Stretch this window to the size of the group
ui.stretch.accesskey=r

ui.shrink.short=Shrink
ui.shrink.long=Shrink this window to the original size
ui.shrink.accesskey=r
